...
*/
    ...
    */
        ...
        try {
            ...
            if (msg.status === 'success') {
                this.log.info(
                    `Modem opened successfully, Port: ${JSON.stringify(msg.data.modem)} Status: ${JSON.stringify(
                        msg.data.status,
                    )}`,
                );
            } else {
            ...
            this.log.warn(
                `Error opening - ${JSON.stringify(err)} - pls check your settings (port, serial connection) and restart the instance`,
            );
            this.setState('info.error', JSON.stringify(err), true);
        }
    } //end modemInitialize()
  ...
  */
    ...
    async phonebook(id, ownPhone) {
        //Check mit nachfolgender Abfrage einbauen
        try {
    ...
    async sending(messageRawJson) {
        try {
            // Finally send an SMS
            const result = await gsmModem.sendSMS(
                messageRawJson.recipient,
                messageRawJson.message,
                messageRawJson.alert,
            );
            ...
            switch (id) {
                ...
                case 'admin.opMode':
                    ...
                    if (state.val === 'PDU' || state.val === 'SMS') {
                        ...
                        this.log.warn(
                            `Operating mode has to be set to 'PDU' (default) or 'SMS', you entered ${state.val}`,
                        );
                        return;
                    }
                    break;
